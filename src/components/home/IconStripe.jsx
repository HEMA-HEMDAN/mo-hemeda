    ...
    <div className="relative">
      ...
      <div className="block md:hidden relative overflow-hidden h-24">
        <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[135vw] bg-white text-[#1b232e] rotate-2 px-10 py-6 select-none origin-center shadow-xl">
          <div className="whitespace-nowrap flex items-center">
            <div ref={mobileTrackRef} className="inline-flex gap-10 text-xl will-change-transform">
              {Array.from({ length: 2 }).map((_, idx) => (
                <React.Fragment key={idx}>
                  {icons.map((ic, i) => (
                    <span key={`${idx}-m-${i}`} className="opacity-90">{ic}</span>
                  ))}
                  {icons.map((ic, i) => (
                    <span key={`dup-${idx}-m-${i}`} className="opacity-90">{ic}</span>
                  ))}
                </React.Fragment>
              ))}
            </div>
      ...
      <div className="hidden md:block relative overflow-visible">
        <div className="relative left-1/2 -translate-x-1/2 w-[130vw] bg-white text-[#1b232e] rotate-2 px-16 py-8 select-none origin-center shadow-xl">
          <div className="whitespace-nowrap flex items-center">
            <div ref={desktopTrackRef} className="inline-flex gap-12 text-2xl will-change-transform">
              {Array.from({ length: 2 }).map((_, idx) => (
                <React.Fragment key={idx}>
                  {icons.map((ic, i) => (
                    <span key={`${idx}-d-${i}`} className="opacity-90">{ic}</span>
                  ))}
                  {icons.map((ic, i) => (
                    <span key={`dup-${idx}-d-${i}`} className="opacity-90">{ic}</span>
                  ))}
                </React.Fragment>
              ))}
            </div>
